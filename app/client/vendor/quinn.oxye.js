...
(function ($, _) {
    jQuery.fn.oxyeSlider = function(params) {
        ...
        createLabels = function() {
          for( var i=0; i < handles.length + 1; i++) {
            valueLabel = $("<span style='position:absolute;z-index:100' class='value-label'>label<span>");
            bar.append(valueLabel);
            valueLabels.push(valueLabel);
          }

          for( var i=0; i < params.labels.length; i ++ ){
            textLabel = $("<span style='position:absolute;z-index:100' class='text-label'>" + params.labels[i] + "<span>");
            bar.append(textLabel);
            textLabels.push(textLabel);
          }
        };

        drawLabels = function(values, options) {
              ...
              for(var i = 0; i < values.length + 1; i++) {
                if( i === 0 ){
                  ...
                  if ( i < values.length ){
                    ...
                    currentValue = params.max; //+ params.minRange;
                  }
                  realValues.push(currentValue-values[i-1]);
                }
              }
           }
          ...
          for(var i = 0; i < valueLabels.length; i++) {
            if( i === 0 ) {
              startCoord = bar.position().left;
            }
            else {
              startCoord = handles[i-1].position().left; //+ handles[i-1].width()/2;
            }

            if( i === (valueLabels.length-1) ){
              endCoord = bar.position().left + bar.width();
            }
            else {
              endCoord = handles[i].position().left;// - handles[i].width()/2; 
            }

            if(reCalculateValues){
                valueLabels[i].html(realValues[i] + " руб.");
            }
            yCoord = startCoord + (endCoord - startCoord) / 2;

            maxWidth = (endCoord - startCoord - 14);

            valueLabels[i].css('max-width', maxWidth);
            valueLabels[i].css('left', yCoord - valueLabels[i].width()/2);
            
            textLabels[i].css('max-width',  maxWidth);
            textLabels[i].css('left', yCoord - textLabels[i].width()/2);
        ...
        drawLabelsThrottled = _.debounce(drawLabels, 10);

        change = function(newValue, slider) {
          result = true;
          drawLabelsThrottled(newValue);
          return result;
        }

        update = function (values) {
            quinnn.model.setValue(values);
            quinnn.renderer.redraw();

            for (var i = 0; i < 10; i++ ){
                setTimeout( function() { drawLabels(values, {recalculate:false}); }, i*30);
            }

            setTimeout( function() { drawLabels(values); }, 100);
        }

        customFunctions = {

          setup : function (currentValue, quinn) {
            quinnn  = quinn;
            handles = quinn.renderer.handles;
            bar     = quinn.renderer.bar;
            drawLabels(params.value);
          },
